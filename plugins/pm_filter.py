...
async def next_page(bot, query):
    ...
    btn.insert(0, [
        InlineKeyboardButton("🔹 How to download 🔹", url='https://t.me/HowToDownload786/13')
    ])
    try:
...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb and imdb.get('poster'):
        try:
            bbb = await message.reply_photo(photo=imdb.get('poster'), caption=cap[:1024],reply_markup=InlineKeyboardMarkup(btn))
            await asyncio.sleep(600)
            await bbb.delete()
        except (MediaEmpty, PhotoInvalidDimensions, WebpageMediaEmpty):
            pic = imdb.get('poster')
            poster = pic.replace('.jpg', "._V1_UX360.jpg")
            bb = await message.reply_photo(photo=poster, caption=cap[:1024], reply_markup=InlineKeyboardMarkup(btn))
            await asyncio.sleep(600)
            await bb.delete()
        except Exception as e:
            logger.exception(e)
            bbc = await message.reply_text(cap, reply_markup=InlineKeyboardMarkup(btn))
            await asyncio.sleep(600)
            await bbc.delete()
    else:
        mbb = await message.reply_text(cap, reply_markup=InlineKeyboardMarkup(btn))
        await asyncio.sleep(600)
        await mbb.delete()
    if spoll:
